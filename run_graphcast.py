...
class DummyDataset(Dataset):
    def __init__(
        ...
        self.dtype = dtype
        self.inputs, self.targets = [None] * num_samples, [None] * num_samples
        for s in range(num_samples):
            x = torch.randn((data_channels, res_h, res_w), dtype=dtype, device=device)
            y = x + torch.randn((data_channels, res_h, res_w), dtype=dtype, device=device) * 0.1
            if s > 0:
                self.inputs[s-1] = x.to(device="cpu")
                self.targets[s-1] = y.to(device="cpu")
        self.inputs[num_samples-1] = x.to(device="cpu")
        self.targets[num_samples-1] = y.to(device="cpu")
    ...
    def __getitem__(self, idx):
        return (
            self.inputs[idx % self.num_samples],
            self.targets[idx % self.num_samples],
        )
...
def run(
    ...
    for batch_id, (batch_inputs, batch_targets) in enumerate(loader):
        epoch = batch_id // num_batches
        batch_id = batch_id % num_batches
        ...
        loss.backward()
        optimizer.step()
        losses.append(loss.detach().to("cpu", non_blocking=True))
...
if __name__ == "__main__":
    ...
    # layers reduced to 12 instead of 16 to fit within 80GB
    parser.add_argument('--processor_layers', type=int, default=12)
